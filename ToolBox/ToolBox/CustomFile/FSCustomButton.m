...
- (void)layoutSubviews
{
    ...
    if (self.buttonImagePosition == FSCustomButtonImagePositionTop || self.buttonImagePosition == FSCustomButtonImagePositionBottom) {
        CGFloat scale = self.imageScale == 0?1:self.imageScale ;
        
        CGFloat imageLimitWidth = (contentSize.width - UIEdgeInsetsGetHorizontalValue(self.imageEdgeInsets) ) * scale;
        CGSize imageSize = [self.imageView sizeThatFits:CGSizeMake(imageLimitWidth, CGFLOAT_MAX)];// 假设图片高度必定完整显示
        if (self.imageScale == 0) {
            if (imageSize.width<24) {
                imageSize.width = 24;
                imageSize.height = 24;
            }
        }
        ...
        switch (self.contentHorizontalAlignment) {//重置X坐标
            ...
            case UIControlContentHorizontalAlignmentCenter:
                imageFrame = CGRectSetX(imageFrame, self.contentEdgeInsets.left + self.imageEdgeInsets.left + CGFloatGetCenter(self.frame.size.width, imageSize.width));
        ...
        if (self.buttonImagePosition == FSCustomButtonImagePositionTop) {//重置Y坐标
            switch (self.contentVerticalAlignment) {
                ...
                case UIControlContentVerticalAlignmentCenter: {
                    ...
                    titleFrame = CGRectSetY(titleFrame, CGRectGetMaxY(imageFrame) + self.imageEdgeInsets.bottom + self.titleEdgeInsets.top+3);
                }
                    break;
        ...
        CGFloat scale = self.imageScale == 0?1:self.imageScale ;
        CGFloat imageLimitHeight = (contentSize.height - UIEdgeInsetsGetVerticalValue(self.imageEdgeInsets)) * scale;
...
- (CGSize)sizeThatFits:(CGSize)size {
    ...
    switch (self.buttonImagePosition) {
        ...
        case FSCustomButtonImagePositionBottom: {
            ...
            CGFloat scale = self.imageScale == 0?1:self.imageScale ;
            CGFloat imageLimitWidth = (contentLimitSize.width - UIEdgeInsetsGetHorizontalValue(self.imageEdgeInsets)) * scale;
            CGSize imageSize = [self.imageView sizeThatFits:CGSizeMake(imageLimitWidth, CGFLOAT_MAX)];// 假设图片高度必定完整显示
        ...
        case FSCustomButtonImagePositionRight: {
            if (self.buttonImagePosition == FSCustomButtonImagePositionLeft && self.titleLabel.numberOfLines == 1) {
                ...
                CGFloat scale = self.imageScale == 0?1:self.imageScale ;
                CGFloat imageLimitHeight = (contentLimitSize.height - UIEdgeInsetsGetVerticalValue(self.imageEdgeInsets)) * scale;
                CGSize imageSize = [self.imageView sizeThatFits:CGSizeMake(CGFLOAT_MAX, imageLimitHeight)];// 假设图片宽度必定完整显示，高度不超过按钮内容
...
- (CGRect)imageRectForContentRect:(CGRect)contentRect{
    ...
    CGFloat imageW = contentRect.size.width * self.imageScale;
